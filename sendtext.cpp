...
{
    textqueue_lock.lock();  //互斥锁上锁
    while(textqueue.size() > QUEUE_MAXSIZE)
    {
        queue_waitCond.wait(&textqueue_lock);  //循环等待
    }
    textqueue.push_back(M(str, msgType));  //写入M

    textqueue_lock.unlock();  //互斥锁解锁
    queue_waitCond.wakeOne();  //唤醒下一个等待线程
}
...
//        qDebug() << "取出队列:" << QThread::currentThreadId();
        ...
        //构造消息体
        MESG* send = (MESG*)malloc(sizeof(MESG));//消息分配内存，并根据消息类型构造消息体
        if (send == NULL)
        {
        ...
        else
        {
			...
			else if (text.type == JOIN_MEETING)
			{
                ...
                else
                {
                    ...
                    memcpy(send->data, &roomno, sizeof(roomno));//内存复制
					//加入发送队列
