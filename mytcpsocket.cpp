...
void MyTcpSocket::stopImmediately()//立即停止套接字的运行
{
    {
        QMutexLocker lock(&m_lock);//锁住m_lock
...
void MyTcpSocket::closeSocket()//关闭套接字
{
...
{
    qRegisterMetaType<QAbstractSocket::SocketError>();//SocketError枚举类型
    _socktcp = nullptr;//_socktcp是QTcpSocket类型的指针，用于后续创建和管理TCP套接字

    _sockThread = new QThread(); //发送数据线程
    this->moveToThread(_sockThread);//将MyTcpSocket对象（this指针表示）移动到新创建的_sockThread线程
	connect(_sockThread, SIGNAL(finished()), this, SLOT(closeSocket()));
    sendbuf =(uchar *) malloc(4 * MB); //为发送缓冲区sendbuf分配内存
    recvbuf = (uchar*)malloc(4 * MB); //为接收缓冲区sendbuf分配内存
    hasrecvive = 0; //用于跟踪接收缓冲区中已经接收的数据量。初始值为0。

}


void MyTcpSocket::errorDetect(QAbstractSocket::SocketError error) //错误处理
{
    qDebug() <<"Sock error" <<QThread::currentThreadId();//打印错误的线程
    ...
    {
        memset(msg, 0, sizeof(MESG));//使用memset函数将MESG结构体的内存初始化为零。
        ...
        queue_recv.push_msg(msg);//无论错误类型是什么，最终都将包含错误信息的消息结构体通过push_msg函数放入到接收队列queue_recv中
    }
}



void MyTcpSocket::sendData(MESG* send)//发送信息
{
    if (_socktcp->state() == QAbstractSocket::UnconnectedState)//检查是否连接上
	{
        emit sendTextOver();
	...
	//消息类型
    qToBigEndian<quint16>(send->msg_type, sendbuf + bytestowrite);//大端转小端
	bytestowrite += 2;
...
bool MyTcpSocket::connectToServer(QString ip, QString port, QIODevice::OpenModeFlag flag)
{
	...
	bool retVal;
    //invokeMethod这是Qt元对象系统中的一个静态方法，用于在运行时调用对象的方法。
    //这种调用方式通常用于跨线程通信，特别是当需要从非GUI线程调用GUI线程的方法时。
    //使用Qt::BlockingQueuedConnection可以确保在继续执行其他任务之前，GUI线程已经处理了方法调用。
